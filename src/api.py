"""FastAPI application for smart-camera-analytics."""
...
app = FastAPI(title="smart-camera-analytics", version="0.1.0", description="smart-camera-analytics API")
...
@app.get("/health")
def health():
    return {"status": "ok", "service": "smart-camera-analytics", "version": "0.1.0"}
...
async def process(req: ProcessRequest):
    ...
    try:
        ...
        resp = ProcessResponse(success=True, result=result, processing_time_ms=round(elapsed, 2),
                              metadata={"service": "smart-camera-analytics", "input_length": len(req.input)})
...
@app.get("/status")
def status():
    return {"service": "smart-camera-analytics", "version": "0.1.0", "ready": True,
